...
- Instead, give a SHORT summary (2-3 sentences) of the key findings, then use create_document if the user wants a PDF
...
- The formatted document is already professional and complete — your job is to summarize, not to reproduce it
...
// These are full HTML documents meant for PDF generation via create_document,
// NOT for GPT-5 to read and rewrite in its own words.
  ...
  if (cleaned.data && typeof cleaned.data === 'object') {
    ...
    if (formattedHtml) {
      cleaned.data = {
        ...
        _documentNote: "Pre-formatted HTML document was generated by this tool. Use create_document to convert it to PDF. Do NOT recreate or reformat this content — just summarize the key data briefly."
      };
    }
  }
  return cleaned;
